/*Funções úteis, implementadas e disponibilizadas pelo monitor da matéria, para a realização do 2° e 3° Trabalho prático de Organização de arquivos - 1° semestre de 2019
  Autor: Matheus Carvalho Raimundo */
...
void scan_quote_string(char *str) {
	...
	*/
	...
	if(R == 'N' || R == 'n') { // campo NULO
		...
		if(R == '\"') {
			if(scanf("%[^\"]", str) != 1) strcpy(str, ""); //ler até fechamento das aspas	
			getchar(); // ignorar aspas fechando
		}else{
			if(R != EOF){ // vc tá tentando ler uma string que não tá entre aspas! Fazer leitura normal %s então...
				str[0] = R;
				scanf("%s", &str[1]);
			}else strcpy(str, "");//EOF
		}
