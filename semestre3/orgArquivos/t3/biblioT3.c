/*Biblioteca de funções utilizadas no 3° Trabalho prático de Organização de arquivos - 1° semestre de 2019
...
//função que atualiza o tamanho do registro lido do arquivo, pois pode ser que o registro lido estaja no fim da página e o tamanho real dele não é o que foi lido
...
int verifcaInconsistencia(FILE *bin, char *auxChar, header *c){
	...
	fread(&c->topoLista, sizeof(long int), 1, bin);//pulando status, encadeamentolista e primeria tag
	//lendo cabeçalho do arquivo 1
	for (int i = 0; i < 5; ++i){
...
//função que carrega todo o arquivo de dados em um vetor na memória principal e retorna esse vetor pra função de ordenação heapsort
...
//função de comparação utilizada para a ordenação do vetor de registros
...
FILE *geraArquivoOrdenado(FILE *bin, char *nomeArquivo){
	...
	//ordenação dos registros com o quicksort
