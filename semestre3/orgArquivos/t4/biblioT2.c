/*Biblioteca de funções utilizadas no 2°, 3° e 4° Trabalho prático de Organização de arquivos - 1° semestre de 2019
...
//função que adiciona um registro removido do arquivo de dados na forma de um registro de índices a um vetor de removidos 
void adicionaRemovidos(dataRegisIndice **removidos, char *nome, long int offset, int *tamRemovidos){
	//guardando o registros removido para poder atualizar o arquivo de índices
	if(nome != NULL){
		(*tamRemovidos)++;
		if((*tamRemovidos) == 1) *removidos = malloc(sizeof(dataRegisIndice));
		else *removidos = realloc(*removidos, sizeof(dataRegisIndice)*(*tamRemovidos));
		strcpy((*removidos)[*tamRemovidos - 1].nome, nome);
		(*removidos)[(*tamRemovidos) - 1].offset = offset;
	}
}

//remove registros com o campo e o valor passados por paramêtro do arquivo binário
dataRegisIndice* removeRegis(FILE *bin, char campo, void *valor, int *tamRemovidos){
	(*tamRemovidos) = 0;//inicializando o tamanho do vetor de removidos com 0
	dataRegisIndice *removidos = NULL;//vetor que armazena os offsets dos registros removidos removidos
		...
		switch(campo){
			case 'i':
				...
				if(regis.removido == '-'){//se o resgistro lido não tiver removido
					if(regis.idServidor == *auxid){//verifica se é que está sendo buscado na remoção
						
						//guardando o registros removido para poder atualizar o arquivo de índices
						adicionaRemovidos(&removidos, regis.nomeServidor, cont - (tamRegis+5), tamRemovidos);
...
//além disso gera um registro do arquivo de índices e retorna ele
dataRegisIndice insercao(FILE *bin, dataRegis regisEscrever){
	dataRegisIndice retorno;//registro do arquivo de índice a ser gerado a partir do registro que vai ser inserido
	
	//se o registros a ser inserido tiver nome copia ele no registro de retorno, caso contrário adiciona -1 ao offset do registro de retorno pra indicar que ele não 
	//deve ser inserido no arquivo de índices
	if(regisEscrever.nomeServidor != NULL) strcpy(retorno.nome, regisEscrever.nomeServidor);
	else {
		strcpy(retorno.nome, "");
		retorno.offset = -1;
	}
	...
	header c;//cabeçalho auxiliar
	dataRegis regis;//auxiliar para ler os registros do fim de uma página de discoe ajustar o tamanho do último registro dessa página
	...
	while(aux != NULL){//enquanto não chegar no fim da lista
	   	if(regisEscrever.tamanhoRegistro <= aux->tamRegis){//se couber em um dos registro da lista
			...
			if(regisEscrever.nomeServidor != NULL) retorno.offset = posInsercao;
    ...
    if(inseriu == false){
		...
		if(regisEscrever.nomeServidor != NULL) retorno.offset = ftell(bin);

		escreveRegis(bin, regisEscrever);
    }

    //escreve a lista de removidos atualizada no arquivo
	...
	return retorno;//retorno o registro doa rquivo de índices
}
...
void copiaRegis(dataRegis *destino, dataRegis fonte){
	...
	for (int j = 0; j < 14; ++j) destino->telefoneServidor[j] = fonte.telefoneServidor[j];
...
//Função que faz a inserção de registro que foi modificado na função de atualização de um campo do registro
