/*Biblioteca de funções utilizadas no 1°, 2°, 3° e 4° Trabalhos práticos de Organização de arquivos - 1° semestre de 2019
...
int printBusca(header c, dataRegis regis){
	if(regis.removido != '-'){
		...
		auxtel[14] = 0;						
		if(regis.telefoneServidor[0] != 0) printf("%s: %s\n", c.desCampos[2], auxtel);
		else printf("%s: valor nao declarado\n", c.desCampos[2]);
...
void search(FILE *bin, char campo, void *valor, int *numPgs){
	...
	fread(&auxChar, sizeof(char), 1, bin);//lendo um caracter pra ver se arquivo não está incosistente
	if(auxChar == '0' || auxChar == 0) {
		printf("Falha no processamento do arquivo.\n");
		fclose(bin);
		exit(0);
	...
	fseek(bin, 9, SEEK_CUR);//pulando status, encadeamentolista
	for (int i = 0; i < 5; ++i){
	...
	//realizando o cálculo do número de páginas de disco acessadas, e mostrando esse valor na tela
	pgs = (double)cont/32000;
	cont -= pgs*32000;
	if(cont > 0) pgs++;//se sobrar algum valor em cont significa que teve acesso a mais uma página
	*numPgs = pgs;
...
//mostra na tela o registro passado
void printRegis(dataRegis *regis){
	printf("%d ", regis->idServidor);
	if(regis->salarioServidor == -1) printf("         ");
	else printf("%.2lf ", regis->salarioServidor);

	if(regis->telefoneServidor[0] == 0) printf("              ");
	else printf("%s ", regis->telefoneServidor);

	if(regis->indicadorDeTamanhoNome != 0) printf("%d %s ", regis->indicadorDeTamanhoNome -2, regis->nomeServidor);
	if(regis->indicadorDeTamanhoCargo != 0) printf("%d %s", regis->indicadorDeTamanhoCargo -2, regis->cargoServidor);
	printf("\n");
}
